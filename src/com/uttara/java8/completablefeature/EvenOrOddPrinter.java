...
public class EvenOrOddPrinter {
	...
	{
		CompletableFuture.runAsync(()->EvenOrOddPrinter.printNumber(oddCond));
		CompletableFuture.runAsync(()->EvenOrOddPrinter.printNumber(evenCond));
		Thread.sleep(1000);
	}
	public static void printNumber(IntPredicate condition)
	{
		IntStream.rangeClosed(1, 10).filter(condition).forEach(EvenOrOddPrinter::execute);
	}
